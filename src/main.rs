...
#[web::get("/user")]
async fn get_users(state: web::types::State<Arc<AppState>>) -> HttpResponse {
	let users = state.db.users().find_many(vec![]).exec().await.unwrap();

	HttpResponse::Ok().json(&users)
}

#[web::post("/user")]
async fn create_user(state: web::types::State<Arc<AppState>>, user: web::types::Json<UserInput>) -> HttpResponse {
	let user = state.db.users().create(user.name.to_string(), user.email.to_string(), vec![]).exec().await.unwrap();

	HttpResponse::Created().json(&user)
}

#[web::delete("/user/{id}")]
async fn delete_user(state: web::types::State<Arc<AppState>>, id: web::types::Path<i32>) -> HttpResponse {
	let user = state.db.users().delete(users::id::equals(id.into_inner())).exec().await.unwrap();
